...
import { quoteStatusInitialState, QuoteStatusState } from './quote-status.state';
...
const QuoteStatusReducer = createReducer(
  ...
  on(QuoteStatusActions.loadQuoteStatusesSucess, (state: QuoteStatusState, { quote_status }) => {
    return {
      ...state,
      quote_status,
    };
...
export function quoteStatusReducer(state = quoteStatusInitialState, action: Action) {
